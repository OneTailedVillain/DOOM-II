...
function doom.addWeapon(wepname, properties)
    ...
    -- If idle isn't present, inherit base idle or create a minimal one
    ...
    if properties.weaponslot == nil then
        error("Weapon '" .. wepname .. "' is missing weaponslot property!")
    elseif properties.weaponslot < 1 or properties.weaponslot > 7 then
        error("Invalid weaponslot " .. tostring(properties.weaponslot) .. " for weapon '" .. wepname .. "'! Must be between 1 and 7.")
    end
...
if properties.statesasslots then
	local statesbackup = deepcopy(properties.states)
    local rawstates = properties.states

    -- Sanitize var1/var2 BEFORE freeslotting
    for stateName, frames in pairs(rawstates) do
        for i, f in ipairs(frames) do
            if f.var1 ~= nil and type(f.var1) ~= "number" then
                print(string.format(
                    "WARNING: %s.%s[%d].var1 is not a number! Forcing to 0.",
                    wepname, stateName, i
                ))
                f.var1 = 0
            end

            if f.var2 ~= nil and type(f.var2) ~= "number" then
                print(string.format(
                    "WARNING: %s.%s[%d].var2 is not a number! Forcing to 0.",
                    wepname, stateName, i
                ))
                f.var2 = 0
            end
    ...
    local freed = FreeDoomStates(wepname, rawstates)

    -- First, handle the automatic terminal frame linking (for states without explicit goto/next)
    for stateName, frames in pairs(rawstates) do
        if #frames > 0 then
            local lastIndex = #frames
            local f = frames[lastIndex]

            -- Only override if the user didn't explicitly direct it elsewhere
            if not f.goto and not f.next then
                local slot = freed[stateName] and freed[stateName][lastIndex]
                if slot and states[slot] then
                    local nextslot

                    if stateName == "lower" or stateName == "raise" then
                        nextslot = freed[stateName][1]
                    else
                        nextslot = freed.idle and freed.idle[1]
                    end

                    states[slot].nextstate = nextslot or S_NULL
                end
            end
        end
    end

    -- Now resolve string gotos to actual slot numbers
    -- Do this AFTER automatic linking so explicit gotos take precedence
    for stateName, frames in pairs(rawstates) do
        for i, f in ipairs(frames) do
            if f.goto and type(f.goto) == "string" then
                -- Convert string goto to the first frame of the target state
                local targetState = rawstates[f.goto]
                if targetState and #targetState > 0 then
                    -- Get the freed slot for the target state's first frame
                    local targetSlot = freed[f.goto] and freed[f.goto][1]
                    if targetSlot then
                        -- Get the slot for this frame and set its nextstate
                        local currentSlot = freed[stateName] and freed[stateName][i]
                        if currentSlot and states[currentSlot] then
                            states[currentSlot].nextstate = targetSlot
                            -- Clear goto to avoid confusion (it's been applied)
                            f.goto = nil
                        end
                    else
                        print(string.format(
                            "WARNING: %s.%s[%d].goto references '%s' which has no valid slot!",
                            wepname, stateName, i, f.goto
                        ))
                        f.goto = nil
                    end
                else
                    print(string.format(
                        "WARNING: %s.%s[%d].goto references '%s' which doesn't exist!",
                        wepname, stateName, i, f.goto
                    ))
                    f.goto = nil
                end
	...
	properties.statesraw = statesbackup
    properties.states = freed
end

    doom.weaponAmmoPickupDef[wepname] = {properties.ammotype, properties.pickupamount or 2}
...
---@field weaponAmmoPickupDef table<string, {[1]: string, [2]: integer, [3]: boolean|nil}> Mapping of weapon names used as ammo sources (weapon pickup bonus stats)

doom.ammonameToPickupDef = {}

doom.weaponAmmoPickupDef = {
    pistol         = {"bullets", 2},
    chaingun       = {"bullets", 2},
    shotgun        = {"shells", 2},
