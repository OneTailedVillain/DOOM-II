...
rawset(_G, "F_StartCast", function()
	...
	if mobjinfo[t].seesound and not doom.IsTitleMode(true) then
...
*/
	...
	if sfx and not doom.IsTitleMode(true) then
...
local function F_CastResponder(keyevent)
    if not caststate or doom.IsTitleMode() then return false end
    ...
    castframes = 0
    castattacking = false
    if mobjinfo[t].deathsound and not doom.IsTitleMode(true) then
