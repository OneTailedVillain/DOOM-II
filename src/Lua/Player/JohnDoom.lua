...
local function P_PlayerMove(player, doRun)
	...
	local mult = properties.movefactor or 2048
	if not doRun then
		mult = FixedMul($, properties.walkfactor or FRACUNIT/2)
	end
...
addHook("MobjDamage", function(target, inflictor, source, damage, damagetype)
	...
	if inflictor then
		if inflictor.preferselfdamage then
			preferselfdamageinfo = true
		end
	end

	DOOM_DamageMobj(target, inflictor, source, inflictor and preferselfdamageinfo and inflictor.doom.damage or damage, damagetype)
	return true
end, MT_PLAYER)
