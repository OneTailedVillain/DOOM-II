...
local function onPickup(item, mobj)
	...
	local pickupfactors = properties.pickupfactors
	local healthfactors = pickupfactors and pickupfactors.health or {}
	local healthgive = healthfactors.all or FRACUNIT
	healthgive = FixedMul($, healthfactors.medikit or FRACUNIT)
	healthgive = FixedMul(25, $)

	local gained = GiveHealthCompat(funcs, player, healthgive, maxhealth)
...
local function onPickup(item, mobj)
	...
	local healthfactors = pickupfactors and pickupfactors.health or {}
	local healthgive = healthfactors.all or FRACUNIT
	healthgive = FixedMul($, healthfactors.stimpack or FRACUNIT)
	healthgive = FixedMul(10, $)

	if GiveHealthCompat(funcs, player, healthgive, maxhealth) then
