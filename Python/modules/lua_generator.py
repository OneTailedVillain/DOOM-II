...
def build_structured_lua_deh(structured_deh: dict) -> bytes:
	...
	for mode, entries in structured_deh.items():
		...
		if mode.upper() == "CODEPTR":
			if "codeptr" not in initialized_tables:
				lines.append('doom.dehacked.bex_codeptrs = doom.dehacked.bex_codeptrs or {}')
				initialized_tables.add("codeptr")
				lines.append('')

			# Special handling for CODEPTR to output the codepointer value
			for entry in entries:
				idx = entry.get('id')
				if idx is None:
					continue
				...
				fields = entry.get('fields', {})
				codepointer = fields.get('CODEPOINTER', b'').decode('latin-1', errors='replace')
				
				if codepointer:
					lines.append(f'doom.dehacked.bex_codeptrs[{idx}] = "{codepointer}"')
				else:
					# Fallback: use the comment from original line
					original_line = fields.get('_ORIGINAL_LINE', b'').decode('latin-1', errors='replace')
					name_match = re.search(r'=\s*(\w+)', original_line)
					if name_match:
						codepointer = name_match.group(1)
						lines.append(f'doom.dehacked.bex_codeptrs[{idx}] = "{codepointer}"')
					else:
						lines.append(f'doom.dehacked.bex_codeptrs[{idx}] = {{}}')
			lines.append('')
			continue

		# Handle SPRITES section
		if mode.upper() == "SPRITES":
			if "sprites" not in initialized_tables:
				lines.append('doom.dehacked.bex_sprites = doom.dehacked.bex_sprites or {}')
				initialized_tables.add("sprites")
				lines.append('')
			...
			for entry in entries:
				...
				fields = entry.get('fields', {})
				sprite_name = fields.get('SPRITE_NAME', b'').decode('latin-1', errors='replace')
				
				if sprite_name:
					# Store as a simple string mapping
					lines.append(f'doom.dehacked.bex_sprites[{idx}] = "{sprite_name}"')
				else:
					# Fallback: try to extract from original line
					original_line = fields.get('_ORIGINAL_LINE', b'').decode('latin-1', errors='replace')
					name_match = re.search(r'=\s*([A-Z0-9_]+)', original_line)
					if name_match:
						sprite_name = name_match.group(1)
						lines.append(f'doom.dehacked.bex_sprites[{idx}] = "{sprite_name}"')
					else:
						lines.append(f'doom.dehacked.bex_sprites[{idx}] = {{}}')
			lines.append('')
			continue

		# Handle SOUNDS section
		if mode.upper() == "SOUNDS":
			if "sounds" not in initialized_tables:
				lines.append('doom.dehacked.bex_sounds = doom.dehacked.bex_sounds or {}')
				initialized_tables.add("sounds")
				lines.append('')
			...
			for entry in entries:
				...
				fields = entry.get('fields', {})
				sound_name = fields.get('SOUND_NAME', b'').decode('latin-1', errors='replace')
				
				if sound_name:
					# Store as a simple string mapping
					lines.append(f'doom.dehacked.bex_sounds[{idx}] = "{sound_name}"')
				else:
					# Fallback: try to extract from original line
					original_line = fields.get('_ORIGINAL_LINE', b'').decode('latin-1', errors='replace')
					name_match = re.search(r'=\s*([A-Z0-9_]+)', original_line)
					if name_match:
						sound_name = name_match.group(1)
						lines.append(f'doom.dehacked.bex_sounds[{idx}] = "{sound_name}"')
					else:
						lines.append(f'doom.dehacked.bex_sounds[{idx}] = {{}}')
			lines.append('')
			continue
