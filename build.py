...
def should_preprocess(file_path):
	...
	return file_path.suffix.lower() == ".lua" or file_path.name in {"S_SKIN", "MUSICDEF", "SOC_PLAY.ini"}
...
def main():
	...
	print(f"  - {output_wad_name if 'output_wad_name' in globals() else DEFAULT_OUTPUT_WAD_NAME} (Pre-packaged WAD PK3)")
	print(f"  - {output_pk3_default_name} (DOOM Engine PK3)")
	print(f"  - {output_mod_default_name} (Modding Example PK3)")
	print(f"  - {DEFAULT_PYTHON_ZIP_NAME} (WAD processing Python scripts)")
	if args.force:
